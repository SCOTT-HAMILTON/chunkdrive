...
use crate::{bucket::Bucket, inodes::directory::Directory, services::service::{ServiceType, Service}, s3::s3::{S3Type,list_files_in_bucket, download_file, upload_file}};
...
fn s3_root_file() -> String { "chunkdrive-root.dat".to_string() }
...
impl GlobalTrait for Global {
    ...
    fn get_direct_block_count(&self) -> usize { self.direct_block_count }
}
        ...
        Some(s3) => {
            ...
            match files {
                Ok(files) => {
                    ...
                    match root_file {
                        Some(f) => {
                            match download_file(&s3, f).await {
                                Ok(stream) => {
                                    let res = tokio::task::spawn_blocking(||{
                                        let mut de = Deserializer::new(stream.into_blocking_read());
                                        let res = match Deserialize::deserialize(&mut de) {
                                            Ok(root) => root,
                                            Err(err) => Err(format!("deserialize error: {}",err.to_string()))
                                        };
                                        res
                                    }).await;
                                    match res {
                                        Ok(v) => v.map_err(|err|{GetS3RootError::CorruptedRoot(err.to_string())}),
